...
def main():
    ...
    for value in test_values:
        try:
            ...
            print(f"   {value} : valeur non trouvée")
    print()
    ...
    print("13. Gestion des erreurs")
    try:
        bst.find_successor(100)  # Valeur non trouvée
    except ValueNotFoundError as e:
        print(f"   Erreur attendue : {e}")
    
    try:
        bst.find_predecessor(100)  # Valeur non trouvée
    except ValueNotFoundError as e:
        print(f"   Erreur attendue : {e}")
    print()
    
    # 14. Performance sur un grand arbre
    print("14. Test de performance")
    import time
    
    large_bst = BinarySearchTree()
    large_values = list(range(1, 1001))  # 1000 valeurs
    ...
    start_time = time.time()
    for value in large_values[:100]:  # Rechercher 100 valeurs
        large_bst.contains(value)
    search_time = time.time() - start_time
    
    print(f"   Insertion de 1000 valeurs : {insert_time:.4f} secondes")
    print(f"   Recherche de 100 valeurs : {search_time:.4f} secondes")
    print(f"   Taille de l'arbre : {len(large_bst)}")
    print(f"   Hauteur de l'arbre : {large_bst.get_height()}")
    print()
