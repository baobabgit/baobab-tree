...
class TestUtilityOperations:
    ...
    def test_get_common_ancestor(self):
        ...
        ancestor = self.operations.get_common_ancestor(self.root, self.left_left, self.left_right)
        assert ancestor is self.left
        
        # Ancêtre commun de la racine et d'une feuille
        ancestor = self.operations.get_common_ancestor(self.root, self.root, self.left_left)
        ...
        # Nœuds non trouvés
        other_node = BinaryTreeNode(10)
        ancestor = self.operations.get_common_ancestor(self.root, other_node, self.left_left)
        assert ancestor is None
        
        # Arbre vide
        ancestor = self.operations.get_common_ancestor(None, self.left_left, self.left_right)
        assert ancestor is None
