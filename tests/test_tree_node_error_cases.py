...
from src.exceptions import CircularReferenceError, InvalidNodeOperationError, NodeValidationError
...
class MockTreeNode(TreeNode):
    ...
    def validate(self) -> bool:
        ...
        if self._parent is not None and self not in self._parent._children:
            raise NodeValidationError("Child not in parent's children list", "parent_child_consistency", self)
        return True
...
class TestTreeNodeErrorCases(unittest.TestCase):
    ...
    def test_set_parent_circular_reference(self):
        ...
        self.assertIn("Setting this parent would create a circular reference", str(context.exception))
