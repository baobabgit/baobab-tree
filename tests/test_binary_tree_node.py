...
class TestBinaryTreeNode:
    ...
    def test_init_with_all_parameters(self):
        ...
        node = BinaryTreeNode(
            15, parent=parent, left=left, right=right, metadata=metadata
        )
    ...
    def test_validate_children_consistency(self):
        ...
        assert "Children list does not match left and right children" in str(
            exc_info.value
        )
    ...
    def test_add_child_max_children(self):
        ...
        assert "Binary tree node cannot have more than 2 children" in str(
            exc_info.value
        )
        assert exc_info.value.operation == "add_child"
